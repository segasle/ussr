...
foreach ($elements as $element) {
    ...
    foreach ($element['content'] as $item) {
        $content .= '<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6"><img src="' . $item['img'] . '" alt="" class="photo"></div>';
        if (isset($item['text'])){
            if (is_array($item['text']) || is_object($item['text'])){
                foreach ($item['text'] as $value) {
                    $ul .= '<li>' . $value['li'] . '</li>';
/*                    foreach ($value['sup'] as $item){
                            $sup .= '<li>' . $item['number'] . '</li>';
                    }*/
                }
            }
        }
...
//$sup = '<ul>'.$sup.'</ul>';
$ul = '<div class="description"><ul>'.$ul.'</ul></div>';
$content .= '</div></div>';
$images .= '</div></div>';
echo $out.$content.$ul.$images;